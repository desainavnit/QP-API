...
//                    'tutor_experties_category_array' => array(),
    ...
    public function get_tutor_only_list(Request $request) {
        $role_tutor = 3;
        $page = $request->page;
        $tutor_id = $request->tutor_id;
        $student_id = $request->student_id;
...
//        $star_ratings = $request->get('search_ratings');
        ...
        if ($user_role_id == "") {
            $response = array(
                'status' => 'failed',
                'message' => 'Please provide user role id.',
            );
            return $this->respondWithStatus($response);
        }
...
//                    'tutor_experties_category_array' => array(),
            ...
            return response()->json(['tutor_list_array' => $tutor_list_array]);
        } else {
            return response()->json(['tutor_list_array' => array()]);
        }
    }
...
//                        return;
            ...
            if (count($get_one_tutor) > 0) {
                ...
                $tutor_data = array(
                    ...
                    'is_paid' => $is_paid,
                    'is_rated' => $is_rated,
                    'is_allow_rating' => $is_allow_rating
//                    'tutor_experties_category_array' => array()
                );
...
//            print_r($tutor_list_array);
    ...
    /* for tutor start */
    public function get_one_tutor_detail(Request $request) {
        $role_tutor = 3;        
        $tutor_id = trim($request->tutor_id);              
            ...
            if (empty($student_info)) {
                ...
                if (!empty($check_favourite_tutor)) {
                    ...
                    if ($delete_query_res) {
                        $response = array(
                            'status' => 'success',
                            'message' => 'Tutor removed from your Favourites.',
                            'data' => array(
