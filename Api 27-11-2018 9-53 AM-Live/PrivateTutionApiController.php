...
//        } 
            ...
            if (empty($student_info)) {
                ...
                try {
                    ...
                    $response = array(
                        'status' => 'success',
                        'message' => 'Your private tuition request has been sent to ' . $tutor_info->first_name . ' ' . $tutor_info->last_name . ' Once the request has been accepted you will receive a confirmation by email.'
                    );
                    return $this->respondWithStatus($response);
                } catch (Exception $e) {
                    DB::rollback();
        ...
        if ($tutor_id == "") {
            ...
            if (!empty($tutor_info)) {
                ...
                $private_tution = DB::table('private_tuitions as pt')
                        ...
                        ->select('pt.*', 'u.first_name', 'u.last_name','u.image_profile')
                ...
                if (!empty($private_tution)) {
                    foreach ($private_tution as $pt_row) {
                        ...
                        if ($pt_row->status == 1) {
                         ...
                         $buyer_image="";
                        if($pt_row->image_profile !=NULL){
                            $buyer_image=url($pt_row->image_profile);
                        }
                        $tutor_private_tution_array[] = array(
                            'tution_id' => $pt_row->id,
                            'title' => $pt_row->title,
                            'description' => $pt_row->first_name . " " . $pt_row->last_name,
                            'imageUrl' => $buyer_image, 
