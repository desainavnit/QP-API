...
//            print_r($course_q_as_first_step);
    ...
    function get_course_lesson(Request $request) {
        ...
        if (!empty($course_detail)) {
            ...
            /* check course is purchase or not code start */
            $course_order_status = DB::table('orders')
                    ->select("orders.*")
                    ->where("student_id", '=', $student_id)
                    ->where("course_id", '=', $course_id)
            ...
            /* check course is purchase or not code end */
    ...
    function get_one_lesson(Request $request) {
        ...
        $lesson_row = DB::table('course_lessons')
                ->join('courses as c', 'course_lessons.course_id', '=', 'c.id')
                ->select(['course_lessons.*', 'c.title as course_title'])
                ->where('course_lessons.id', '=', $lesson_id)
                ->where('course_lessons.deleted_at', '=', NULL)
                ->first();
        if (!empty($lesson_row)) {
            $course_vid_title = strtolower($lesson_row->course_title);
            $course_vid_title = str_replace(" ", "-", $lesson_row->course_title);
            $url_str = "";
            if (strpos(urldecode($lesson_row->video_s3_name), 'courses/') === false) {
                $url_str = "courses/" . $lesson_row->course_id . "-" . strtolower($course_vid_title) . "/lesson_video/" . urldecode($lesson_row->video_s3_name);
            } else {
