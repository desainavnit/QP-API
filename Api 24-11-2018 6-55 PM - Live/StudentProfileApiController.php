...
//                        ->where('pt.start_date', '<=', date("Y-m-d", strtotime($month_end_date)))
                ...
                foreach ($private_tution as $pt_row) {
                    ...
                    if ($pt_row->status == 1) {
                        ...
                        $status_color = "#14CCA2";
                        if ($pt_row->payment_status == 0) {
                            $tmp_payment_amount = ($pt_row->price_per_h * $pt_row->duration);
                            $tmp_payment_amount = number_format($tmp_payment_amount, 2, ".", "");
                            $total_payment_amount +=$tmp_payment_amount;
                            $total_payment_amount = number_format($total_payment_amount, 2, ".", "");

                           array_push($tution_ids, $pt_row->id);
                        }else{
        ...
        if ($student_id == "") {
            ...
            if (!empty($student_info)) {
                ...
                if (!empty($notification_setting_result)) {
                    if ($notification_setting_result->notification_setting == 1) {
